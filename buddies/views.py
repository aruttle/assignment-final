...
def session_detail(request, pk):
    ...
    return render(request, "buddies/detail.html", {"sess": sess, "messages": messages, "joined": joined})
...
def toggle_join(request, pk):
    ...
    if joined:
        ...
        if sess.spots_left <= 0:
            # pass joined along (still False)
            return render(request, "buddies/partials/_join_box.html", {"sess": sess, "joined": joined, "error": "Session is full."})
        BuddyParticipant.objects.get_or_create(session=sess, user=request.user)
...
def post_message(request, pk):
    ...
    if not body:
        return render(request, "buddies/partials/_message_item.html", {"msg": None, "error": "Message required."})
